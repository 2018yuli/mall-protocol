$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: getRootPath() + 'admin/categories/list?categoryLevel=' + categoryLevel + '&parentId=' + parentId,
        datatype: "json",
        colModel: [
...
function categoryManage() {
    ...
    if (categoryLevel == 1 || categoryLevel == 2) {
        ...
        window.location.href = getRootPath() + 'admin/categories?categoryLevel=' + categoryLevel + '&parentId=' + id + '&backParentId=' + parentId;
    } else {
        swal("无下级分类", {
            icon: "warning",
        });
...
function categoryBack() {
    ...
    if (categoryLevel == 2 || categoryLevel == 3) {
        categoryLevel = categoryLevel - 1;
        window.location.href = getRootPath() + 'admin/categories?categoryLevel=' + categoryLevel + '&parentId=' + backParentId + '&backParentId=0';
    } else {
        swal("无上级分类", {
            icon: "warning",
        });
...
$('#saveButton').click(function () {
    ...
    if (!validCN_ENString2_18(categoryName)) {
        ...
        var url = getRootPath() + 'admin/categories/save';
        var id = getSelectedRowWithoutAlert();
        if (id != null) {
            url = getRootPath() + 'admin/categories/update';
            data = {
                "categoryId": id,
                "categoryName": categoryName,
                "categoryLevel": categoryLevel,
    ...
    swal({
        ...
        dangerMode: true,
            ...
            if (flag) {
                $.ajax({
                    type: "POST",
                    url: getRootPath() + "admin/categories/delete",
                    contentType: "application/json",
